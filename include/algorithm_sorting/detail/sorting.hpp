...
std::size_t checked_range_size(Int min_value, Int max_value) {
	...
	const Unsigned range = static_cast<Unsigned>(max_value) - static_cast<Unsigned>(min_value);
	if (range > static_cast<Unsigned>(std::numeric_limits<std::size_t>::max() - 1)) {
...
std::size_t counting_index(Int value, Int min_value, std::size_t range_size) {
	using Unsigned = typename std::make_unsigned<Int>::type;
	const Unsigned index = static_cast<Unsigned>(value) - static_cast<Unsigned>(min_value);
	if (index >= static_cast<Unsigned>(range_size)) {
